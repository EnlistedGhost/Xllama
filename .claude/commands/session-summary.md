...
- Summarize in 1 sentence using generic terms (e.g., "Added CUDA kernel for a new GGML operation")

### 1b. Skills & Commands Used
- List all slash commands invoked during the session
- List all tools used (by category, not individual calls)
- Note any manual workflows that weren't covered by existing commands

### 1c. Artifacts Produced
- Count of files created/modified (by type: source files, test cases, docs, configs, etc.)
...
- **Fix** — Debugging and fixing issues
- **Build** — Compilation, Docker builds, environment setup
- **Test** — Writing or running tests
- **Trace** — Investigating code paths, understanding execution flow
- **Profile** — Performance measurement and optimization
- **Annotate** — Adding verified comments and documentation
- **Plan** — Breaking down work into issues and tasks
...
- Example: "Built native, then Docker, then ran tests" → candidate for combined command
